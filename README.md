...
В ходе стажировки (формат более напоминает простую учебную практику) мы работали в [общем репозитории](https://github.com/banzarkhan/motorika-internship-team), где можно найти прежние версии некоторых из моих ноутбуков, в том числе с попытками набрести на оптимальное решение, исползуя Keras (не включено в настоящий репозиторий: в настоящее время активно изучаю pytorch и размышляю о способах применения CNN).

В нашей команде мы пытались разрабатывать параллельные решения, обменивались интересным опытом и сообща преодолевали некоторые из насущных проблем: работа с нейросетями для нас всех было чем-то новым и мы не сразу разобрались с тем, в каком виде будет приемлемо подавать данные в RNN-сети. Особый интерес к нейронкам проявила Арюна и ей в конечном итоге удалось создать довольно хорошее решение с GRU — наша команда также благодаря ей смогла осуществить хорошую координацию в Git и Notion. А наш тимлид, Павел Новиков, посещавший за нас встречи со специалистами Моторики в Сколково, сумел продемонстрировать там модель, заработавший на онлайн-инференсе (у другой команды этого, увы, не получилось).
